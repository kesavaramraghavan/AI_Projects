...
class SimpleMetrics:
    """In-process metrics collector (swap for Prometheus in prod)."""
...
def text_fingerprint(text: str) -> str:
    """SHA-256 fingerprint for dedup / caching."""
...
# ─── FastAPI Health / Middleware Helpers ───────────────────────────────────────
...
def load_env_config(prefix: str = "") -> Dict[str, str]:
    """Pull all env vars with optional prefix into a dict."""
    result = {}
    for k, v in os.environ.items():
        if prefix and not k.startswith(prefix):
            continue
        key = k[len(prefix):].lower() if prefix else k.lower()
        result[key] = v
    return result